...
Source0:        cloud-init-1053.tar.gz
Source1:        cloud-init.cfg
Source2:        cloud-init.README
Source3:        cloud-init.conf
Patch0:         cloud_init.patch
Patch1:         cloud_init_dhcp.patch
Patch2:         cloud_init_disable_root_opts.patch

BuildArch:      noarch
BuildRoot:      %{_tmppath}/cloud-init
...
cp -p %{SOURCE2} trunk/cloud-init.README
...
cd trunk
%{__python} setup.py install -O1 --skip-build --root $RPM_BUILD_ROOT --init-system=sysvinit
...
#
mkdir -p $RPM_BUILD_ROOT/%{_usr}/lib/tmpfiles.d
install -p -m 644 %{SOURCE3} $RPM_BUILD_ROOT/%{_usr}/lib/tmpfiles.d/
...
%files
%doc trunk/ChangeLog trunk/LICENSE trunk/cloud-init.README
...
%{_bindir}/cloud-init*
%doc %{_datadir}/doc/cloud-init
%dir %{_sharedstatedir}/cloud

%config(noreplace) %{_sysconfdir}/rsyslog.d/21-cloudinit.conf
%config(noreplace) %{_usr}/lib/tmpfiles.d/cloud-init.conf
